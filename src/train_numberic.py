    ...
    def fit(self, train_loader, raw_loader, num_epochs, val_loader, checkpoint_dir, save_every_n_epochs):
        ...
        with torch.no_grad():
            for batch in self.raw_loader:
                ...
                batch_mae = 0.0
                for y_pred, y in zip(output_floats, batch['y']):
                    median_pred = np.median(y_pred)
                    y_true = y.item()
                    mae = abs(median_pred - y_true)
                    batch_mae += mae
                    print(f"Pred: {median_pred:.6f}, True: {y_true:.6f}, MAE: {mae:.6f}")
                
                batch_avg_mae = batch_mae / len(batch['y'])
                total_mae += batch_avg_mae
                num_batches += 1
                
            
            overall_mae = total_mae / num_batches if num_batches > 0 else 0.0
            print(f"Overall MAE: {overall_mae:.6f}")

        return {"train_loss": total_loss / len(self.train_loader)}
    
    def eval(self, raw_loader, pretrain_ckpt_path, train_dataset):
        ...
        json_filename = f"eval_Numberic_best_results_{timestamp}.json"
