...
class EncoderDecoder(nn.Module):
    ...
    def mean_pooling(self, memory: torch.Tensor, padding_mask: torch.Tensor = None) -> torch.Tensor:
        """
        对embedder后的矩阵进行mean pooling操作，忽略padding位置
        
        Args:
            memory: 输入张量，形状为 (batch_size, seq_len, d_model)
            padding_mask: padding掩码，形状为 (batch_size, seq_len)，True表示padding位置
            
        Returns:
            经过mean pooling的张量，形状为 (batch_size, d_model)
        """
        if padding_mask is None:
            # 如果没有提供padding_mask，直接进行平均池化
            return torch.mean(memory, dim=1)
        
        # 创建mask，True表示有效位置（非padding）
        valid_mask = ~padding_mask  # (batch_size, seq_len)
        
        # 将padding位置的embedding设为0
    ...
    def forward(self, src: torch.Tensor, tgt_input: torch.Tensor, number_mask: torch.Tensor=None) -> torch.Tensor:
        ...
        if number_mask is not None:
            # 对数变换: v = sign(n) * log(1 + |n|)
            transformed_mask = torch.sign(number_mask) * torch.log1p(torch.abs(number_mask))
            
            # 创建傅里叶位置编码
            d_model = self.src_tok_emb.embedding_dim
            positions = torch.arange(src.size(1), device=src.device).unsqueeze(1)
            div_term = torch.exp(torch.arange(0, d_model, 2, device=src.device) * (-math.log(10000.0) / d_model))
            ...
            # 将数值信息融入位置编码
            # 扩展transformed_mask以匹配pe的维度
            value_pe = transformed_mask.unsqueeze(-1) * pe
            
            # 只在number_mask非零的位置应用数值编码
