...
class PyTorchModel(nn.Module, model_base.Model[Tensor]):
    ...
    def compute_loss_and_metrics(
        ...
        if "number_mask" in examples:
            logits = self.encoder_decoder.forward(
                examples["encoder_input"], examples["decoder_input"], examples["number_mask"]
            )
        else:
            logits = self.encoder_decoder.forward(
    ...
    @torch.no_grad()
    def get_mean_pooling_embedding(
    ...
    def convert_inputs(self, inputs: Sequence[core.ExampleInput]) -> dict[str, Tensor]:
        ...
        # global max_len
        # tmp_max = max_len
        # max_len = max(max_len, max(len(t) for t in encoder_token_ids))
        # all_lengths.extend([len(t) for t in encoder_token_ids])
        # if max_len > tmp_max:
        #     print(max_len)
        #     # print(all_lengths)
        #     import pandas as pd
        #     import matplotlib.pyplot as plt
        #     lengths_df = pd.DataFrame(all_lengths, columns=["token_length"])
        #     print(lengths_df.describe(percentiles=[.90, .95, .98, .99]))
    ...
    def convert_number_inputs(self, inputs: Sequence[core.ExampleInput]) -> dict[str, Tensor]:
        ...
        # 创建number_mask：在<num>位置填入对应数值，其他位置为0
        ...
        number_mask[num_positions] = torch.tensor([n for nums in numbers_list for n in nums])
        
        return {
            "encoder_input": encoder_tensor,
            "number_mask": number_mask
        }
    
    def convert_number_examples(self, examples: Sequence[core.Example]) -> dict[str, Tensor]:
